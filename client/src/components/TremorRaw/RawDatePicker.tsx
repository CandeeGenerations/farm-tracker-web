...
const formatDate = (date: Date, locale: Locale, includeTime?: boolean): string => {
  ...
  if (includeTime) {
    dateString = usesAmPm ? format(date, 'MM/dd/yyyy h:mm a', {locale}) : format(date, 'MM/dd/yyyy HH:mm', {locale})
  } else {
    dateString = format(date, 'MM/dd/yyyy', {locale})
  }
          ...
          <div className="flex flex-col overflow-x-scroll sm:flex-row sm:items-start">
            ...
            <div className="overflow-x-scroll">
              <CalendarPrimitive
                ...
                classNames={{
                  months: 'flex flex-row divide-x divide-gray-300 overflow-x-scroll',
                }}
                {...props}
              />
              {showTimePicker && (
              ...
              <div className="border-t border-gray-300 p-3 sm:flex sm:items-center sm:justify-between">
                ...
                <div className="mt-2 flex items-center gap-x-2 sm:mt-0">
                  <RawButton variant="secondary" className="h-8 w-full sm:w-fit" type="button" onClick={onCancel}>
                    {translations?.cancel ?? 'Cancel'}
                  </RawButton>
                  <RawButton variant="primary" className="h-8 w-full sm:w-fit" type="button" onClick={onApply}>
                    {translations?.apply ?? 'Apply'}
                  </RawButton>
                </div>
              </div>
...
const RawDatePicker = ({presets, ...props}: SingleDatePickerProps) => {
  if (presets) {
...
RawDatePicker.displayName = 'DatePicker'
...
const RawDateRangePicker = ({presets, ...props}: RangeDatePickerProps) => {
  if (presets) {
    validatePresets(presets, props)
  }
...
RawDateRangePicker.displayName = 'DateRangePicker'

export {RawDatePicker, RawDateRangePicker, type DatePreset, type DateRangePreset}
